...
name: grok-search
description: |
  Enhanced web search and real-time content retrieval via Grok API. Prefer this skill only when: (1) the user explicitly requests `grok-search` / Grok search, (2) the runtime has no usable built-in web search, or (3) built-in web search is insufficient and multi-source Grok retrieval is specifically needed. It can also handle webpage content extraction and URL parsing. Do not treat this skill as the default replacement for every built-in WebSearch/WebFetch.
allowed-tools: Bash(python:*), Bash(python3:*), Bash(uv:*), Read, Grep
---

# Grok Search

Use this skill for web search, webpage retrieval, and current-information lookups when Grok-backed retrieval is explicitly desired or when the runtime lacks a good built-in web search.

## Rules

- If the current runtime already has a built-in web search, prefer the built-in search by default.
- Switch to this skill only when the user explicitly asks for `grok-search` / Grok search, or when built-in search is unavailable / clearly insufficient.
- Always call `python "<SKILL_DIR>/scripts/groksearch_entry.py" ...`.
- Do not call `scripts/groksearch_cli.py` directly.
- Do not create or rely on `<SKILL_DIR>/.env` in the repository. Secrets are process environment only.
- Default dependency venv lives outside the repo at `~/.cache/hermes/grok-search-venv`; override with `GROKSEARCH_VENV_DIR` if needed.
- Required process env: `GROK_API_URL`, `GROK_API_KEY`.
...
```bash
GROK_API_URL="https://..." GROK_API_KEY="..." python "<SKILL_DIR>/scripts/groksearch_entry.py" web_search --query "search terms" [--platform "GitHub"] [--min-results 3] [--max-results 10] [--model "grok-4.20-auto"] [--extra-sources 3]
...
|----------|----------|-------------|
| User explicitly asks for Grok search | built-in `WebSearch` | CLI `web_search` via `groksearch_entry.py` |
| User explicitly asks for Grok fetch | built-in `WebFetch` | CLI `web_fetch` via `groksearch_entry.py` |
| Runtime has no built-in web search | N/A | CLI `web_search` / `web_fetch` via `groksearch_entry.py` |
