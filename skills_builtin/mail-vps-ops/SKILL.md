---
name: mail-vps-ops
description: 当用户要求读取邮件 VPS 上的邮箱列表、最新邮件、验证码、验证链接、附件，或执行发信、回复、转发、移到Trash、删除邮件时，必须使用本技能。列出全部邮箱时查询所有域名，不默认添加 --domain；只有用户明确指定域名时才过滤。支持通过 QQ 把附件发回给用户。
---
...
列出全部邮箱。用户说“有哪些邮箱”“邮箱列表”“列出邮件 VPS 里的邮箱”而没有指定域名时，必须使用下面的不带过滤命令；不要自行添加 `--domain` 或 `--limit`，也不需要追问完整邮箱地址：

```bash
/home/hermes/.hermes/bin/mail_vps_fetch.py list-mailboxes
```

只有用户明确限定某个域名时，才使用域名过滤，例如用户问“列出 sru.edu.kg 的邮箱”：

```bash
/home/hermes/.hermes/bin/mail_vps_fetch.py list-mailboxes --domain sru.edu.kg
```

按返回的 `mailboxes` 完整列出结果，可按邮箱地址的域名分组。若 `matched` 大于返回列表长度，说明结果被截断，不能声称已经列出全部，应去掉 `--limit` 后重新查询。不要从服务主机名、历史邮箱或后面的示例地址推断查询范围，也不要把一个域名下没有结果解释为整台 VPS 没有邮箱。
...
1. 读取或操作具体邮箱时，用户没说邮箱地址才追问完整邮箱地址；列出邮箱列表不要求邮箱地址，未指定域名时查全部域名。
2. 读邮件默认只看最新 1 封。
