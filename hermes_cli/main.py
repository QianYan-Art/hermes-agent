...
# precedence used everywhere else before UI pruning.
...
def _wants_tui_early(argv: "list[str] | None" = None) -> bool:
    """Tangyuge fork has no TUI surface."""
    return False


# Legacy mouse-tracking residue suppression. Tangyuge disables the TUI launch
# path, so this normally returns before touching the terminal.
def _suppress_mouse_residue_early() -> None:
    if not _wants_tui_early():
        return
    try:
        # Skip when stdout is redirected.
...
def _print_tui_exit_summary(
    ...
    print(f"  hermes --resume {target}")
    if title:
        print(f'  hermes -c "{title}"')
    print()
...
def _resolve_use_tui(args) -> bool:
    """Tangyuge fork always uses the classic CLI chat surface."""
    return False
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    """No-op: web UI surface is disabled in this fork."""
    return True


def _desktop_dist_exists(desktop_dir: Path) -> bool:
    """No-op: desktop surface is disabled in this fork."""
    return False
...
def _kill_stale_dashboard_processes(*args, **kwargs) -> int:
    """No-op: dashboard surface is disabled in this fork."""
    return 0


def _print_curator_first_run_notice() -> None:
    """No-op: dashboard curator notices are disabled in this fork."""
    return None
...
def _update_node_dependencies() -> None:
    """No-op: Node-managed UI workspaces are disabled in this fork."""
    return None
...
def _ensure_fhs_path_guard() -> None:
    ...
    Mirrors the old post-symlink probe so that
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        # build. Desktop packaging is disabled in this fork.
        ...
        # never use the disabled desktop surface shouldn't be forced into a full
        ...
        if (desktop_dir / "package.json").exists() and shutil.which("npm") and has_desktop_app:
            ...
            if build_result.returncode != 0:
                build_result = subprocess.run(_desktop_build_cmd, cwd=PROJECT_ROOT, check=False)
            if build_result.returncode != 0:
                print("  Desktop build skipped; desktop surface is disabled in this fork.")
...
def _report_dashboard_status() -> int:
    """Dashboard surface is disabled in the Tangyuge server fork."""
    print("Dashboard surface is disabled in this Tangyuge server build.")
    return 0

def cmd_dashboard(args):
    """Dashboard surface is disabled in the Tangyuge server fork."""
    print("The dashboard surface is disabled in this Tangyuge server build.")
    sys.exit(2)


def cmd_disabled_dashboard_registration(args):
    """Dashboard registration is disabled in the Tangyuge server fork."""
    print("Dashboard registration is disabled in this Tangyuge server build.")
    sys.exit(2)
...
def _try_termux_fast_tui_launch() -> bool:
    """No-op: TUI surface is disabled in this fork."""
    return False
