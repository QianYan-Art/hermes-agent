...
def delegate_task(
    ...
    # Async delegation is single-task only. Batch background work needs one
    # handle/result per task; reject clearly instead of silently changing mode.
    background = (
    ...
    if background and tasks and isinstance(tasks, list) and len(tasks) > 1:
        return tool_error(
            ...
            "subagent per delegate_task call, or run the batch synchronously "
    ...
    if background and len(task_list) > 1:
        return tool_error(
            "background=true is single-task only. Dispatch one background "
            "subagent per delegate_task call, or run the batch synchronously "
            "with background=false."
    ...
    if n_tasks == 1:
        ...
        if background:
            ...
            # A background child must not be interrupted/closed by the parent
            # turn lifecycle. Its lifecycle is owned by async_delegation.
            if hasattr(parent_agent, "_active_children"):
                try:
                ...
                return json.dumps(
                    {
                        ...
                        "mode": "background",
                        "note": (
                            "Subagent is running in the background. Continue "
                            "working; the full task source and result will "
                            "re-enter the conversation as a new message."
                        ),
                    },
                    ensure_ascii=False,
                )
            try:
                ...
                logger.debug("Failed to close rejected async child")
            return tool_error(
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "background": {
                "type": "boolean",
                "description": (
                    "Run the subagent asynchronously in the background instead "
                    "of blocking this turn. When true, delegate_task returns "
                    "immediately with a delegation_id, and the full result "
                    "re-enters the conversation as a new message when the child "
                    "finishes. Single-task only; cannot be combined with a "
                    "multi-item tasks array."
                ),
            },
            "acp_command": {
                "type": "string",
